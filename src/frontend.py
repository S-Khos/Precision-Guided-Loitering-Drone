...
class FrontEnd(object):
    ...
    def update(self):
        ...
        cv2.putText(self.state.frame, "SPD  {} CM/S".format(self.state.z_speed), ((self.state.CENTRE_X) - 90 -
                    spd_size, (self.state.CENTRE_Y) - 100), self.state.FONT, self.state.FONT_SCALE, self.state.UI_COLOUR, self.state.LINE_THICKNESS)

        cv2.putText(self.state.frame, "ALT  {:.1f} FT".format(self.state.altitude), ((
        ...
        cv2.putText(self.state.frame, "BRM  {}".format(self.state.barometer),
                    (5, self.state.FRAME_HEIGHT - 100), self.state.FONT, self.state.FONT_SCALE, self.state.UI_COLOUR, self.state.LINE_THICKNESS)
        cv2.putText(self.state.frame, "SPD  {}  {}  {}".format(self.state.x_speed, self.state.y_speed, self.state.z_speed), (5,
                    self.state.FRAME_HEIGHT - 70), self.state.FONT, self.state.FONT_SCALE, self.state.UI_COLOUR, self.state.LINE_THICKNESS)
        cv2.putText(self.state.frame, "ACC  {}  {}  {}".format(self.state.acceleration_x, self.state.acceleration_y, self.state.acceleration_z),
                    (5, self.state.FRAME_HEIGHT - 40), self.state.FONT, self.state.FONT_SCALE, self.state.UI_COLOUR, self.state.LINE_THICKNESS)
        cv2.putText(self.state.frame, "YPR  {}  {}  {}".format(self.state.yaw, self.state.pitch, self.state.roll), (5,
        ...
        cv2.putText(self.state.frame, "T + {}".format(self.state.flight_time),
        ...
        if (self.state.KC_manual and not self.state.GS_active):
            cv2.rectangle(self.state.frame, (self.state.CENTRE_X - 20, 10),
                          (self.state.CENTRE_X + 29, 28), self.state.UI_COLOUR, -1)
            cv2.putText(self.state.frame, "CTRL", (self.state.CENTRE_X - 20, 25),
            ...
            cv2.putText(self.state.frame, "AUTO", (self.state.CENTRE_X - 20, 25),
                        self.state.FONT, self.state.FONT_SCALE, self.state.BLACK, self.state.LINE_THICKNESS)
        ...
        if self.state.TR_active and not self.state.TR_reset:
            ...
            if (self.state.CENTRE_X > x and self.state.CENTRE_X < x + w and self.state.CENTRE_Y > y and self.state.CENTRE_Y < y + h and not self.state.KC_manual):
                ...
                cv2.rectangle(self.state.frame, (self.state.CENTRE_X - (lock_size // 2), self.state.FRAME_HEIGHT - 38),
                              (self.state.CENTRE_X + lock_size - 25, self.state.FRAME_HEIGHT - 20), self.state.UI_COLOUR, -1)
                cv2.putText(self.state.frame, "LOCK", (self.state.CENTRE_X - (lock_size // 2),
                ...
                cv2.rectangle(self.state.frame, (self.state.CENTRE_X - (trk_size // 2), self.state.FRAME_HEIGHT - 38),
                              (self.state.CENTRE_X + trk_size - 20, self.state.FRAME_HEIGHT - 20), self.state.UI_COLOUR, -1)
                cv2.putText(self.state.frame, "TRK", (self.state.CENTRE_X - (trk_size // 2),
                            self.state.FRAME_HEIGHT - 22), self.state.FONT, self.state.FONT_SCALE, self.state.BLACK, self.state.LINE_THICKNESS)

            cv2.line(self.state.frame, (x, y), (x + 20, y),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x, y), (x, y + 20),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x, y + h), (x, y + h - 20),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x, y + h), (x + 20, y + h),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)

            cv2.line(self.state.frame, (x + w, y), (x + w - 20, y),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x + w, y), (x + w, y + 20),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x + w, y + h), (x + w, y + h - 20),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)
            cv2.line(self.state.frame, (x + w, y + h), (x + w - 20, y + h),
                     self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, 2)

            cv2.circle(self.state.frame, (x + w // 2, y + h // 2), 3,
                       self.state.RED if self.state.GS_dive else self.state.UI_COLOUR, -1)
            # top
            cv2.line(self.state.frame, (x + w // 2, y),
                     (x + w // 2, 0), self.state.UI_COLOUR, 1)
            # left
            cv2.line(self.state.frame, (x, y + h // 2),
                     (0, y + h // 2), self.state.UI_COLOUR, 1)
            # right
            cv2.line(self.state.frame, (x + w, y + h // 2),
                     (self.state.FRAME_WIDTH, y + h // 2), self.state.UI_COLOUR, 1)
            # bottom
            cv2.line(self.state.frame, (x + w // 2, y + h),
